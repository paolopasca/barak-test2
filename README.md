...
2. **Quantificatori in (17) e (18).** Il paper scrive "$\forall i$ and $p \in O_{ip}; \forall j$ and $r \in O_{jr}; \forall t$" ma il vincolo non contiene alcun indice temporale $t$. Il $\forall t$ spurio è stato rimosso per chiarezza; trattare il vincolo come quantificato su coppie distinte di operazioni $(O_{ip}, O_{jr})$ che condividono almeno una macchina ammissibile.
...
7. **Idle dell'operatore in $Q_2$.** Il primo termine $C_{\max} - \sum W^{hm}_{ip} P^{hm}_{ip}$ assume implicitamente che ogni operatore sia "disponibile" per l'intero orizzonte $[0, C_{\max}]$ e paghi il costo $\text{Cost}_h$ per unità di tempo per ogni tempo *non* speso a processare. Il tempo di setup stesso non è sottratto — l'operatore è considerato idle durante i setup della macchina in questo termine di costo.
